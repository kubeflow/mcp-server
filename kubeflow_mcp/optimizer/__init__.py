...
CLIENT_TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    "get_successful_trials": (
        "All successful trials with hyperparameters and metrics for comparison."
    ),
    ...
    "create_hpo_experiment": (
        ...
        "loguniform, choice. Algorithms: random, grid. "
...
# global preferred order — no hardcoded list in the core.
...
- HPO with flat params → create_hpo_experiment() — specify objective, search space, algorithm
  ...
  - Algorithms: random (default), grid
  - ALWAYS preview first (confirmed=False), then show preview and wait for approval
- Raw V1beta1Experiment spec → create_experiment_from_spec() — escape hatch for advanced configs

OPTIMIZATION RULES:
- ALWAYS call katib_pre_flight() before creating experiments
- ALWAYS preview before submitting (confirmed=False first)
- Supported algorithms through SDK: random, grid
- Advanced algorithms (tpe, cmaes, hyperband) require create_experiment_from_spec()
